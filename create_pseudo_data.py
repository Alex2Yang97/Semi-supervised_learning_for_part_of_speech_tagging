...
def gen_pseudo_data_by_unlabel(
  ...
  with torch.no_grad():
      for i, batch in enumerate(unlabeled_domain_dataset):
        ...
        for words_i, ishead_i, yhat_i in zip(words, is_heads, y_hat):
          new_words.append(words_i.split()[1: -1])
          ...
          assert len(preds)==len(words_i.split()[1: -1])
  ...
  if save_output:
    with open(output_file, 'w') as fout:
      for top_words_i, top_pseudo_tags_i in zip(top_words, top_pseudo_tags):
        for w, p in zip(top_words_i, top_pseudo_tags_i):
          fout.write("{} {}\n".format(w, p))
        fout.write("\n")
  ...
  return (top_words, top_pseudo_tags, top_prob,
          remain_words, remain_pseudo_tags, remain_prob)
